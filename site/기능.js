...
function eventCard(event) {
  ...
  return '<div class="event-row" data-urgency="' + esc(timing.urgency) + '">' +
    ...
    (level ? '<span class="event-level">' + esc(displayStatusLabel(level)) + '</span>' : '') +
    '</div>';
}
...
const DISPLAY_STATUS_LABELS = {
  NEW:"신규", ACTIVE:"진행 중", WATCHING:"관찰", ESCALATING:"강화", EASING:"완화", RESOLVED:"해소",
  CRITICAL:"매우 높음", HIGH:"높음", MEDIUM:"보통", LOW:"낮음", WATCH:"주의", VETO:"매우 높음",
  UNKNOWN:"확인 필요", OK:"정상", NORMAL:"정상", HEALTHY:"정상", ATTENTION:"주의",
  SUCCESS:"성공", FAILURE:"실패", FAILED:"실패", PENDING:"대기", PARTIAL:"일부 확인",
  CHANGED:"변경됨", INVESTIGATING:"조사 중", BLOCKED:"차단됨", STALE:"갱신 지연",
  NO_CHANGE:"변화 없음", RECOVERED:"복구됨", COLLECTING:"수집 중", IMPROVED:"개선됨",
  REVIEWED:"검토 완료", VERIFICATION_PENDING:"검증 대기", RESOLVED_THIS_WINDOW:"이번 구간에서 해결",
  CLOSED:"장 마감", OPEN:"장중", FUTURE:"예정", NOTICE:"확인 필요", UNAVAILABLE:"사용 불가",
  RISK_ON:"위험선호", RISK_OFF:"위험회피", NEUTRAL:"중립", MIXED:"혼조",
  ACTIVE_MARKET_DRIVER:"현재 가격결정", NOT_PRICED:"미반영 가능", EARLY:"초기 반영",
  PARTLY_PRICED:"일부 반영", FULLY_PRICED:"상당부분 반영", LOW_CURRENT_PRICING:"현재 반영 낮음",
  HISTORICAL_FALLBACK:"최근 거래일 기준", OUTSIDE_REGULAR_SESSION:"장 종료", NO_LIVE_DATA:"실시간 데이터 없음",
  NEEDS_CREDENTIALS:"연결 설정 필요", NO_ROWS:"실시간 시세 없음", TOSS_SNAPSHOT_UNAVAILABLE:"토스 수집기 대기",
  OUTSIDE_DOMESTIC_MONITOR_WINDOW:"국내 감시시간 종료"
};
const DISPLAY_IDENTIFIER_LABELS = {
  canonical_window:"정규 관측 구간", sensor_continuity:"센서 연속성", news_verification:"뉴스 검증",
  ui_review:"화면 구성 검토", macro_data_quality:"거시 데이터 품질", news_issues:"뉴스·이슈",
  data_quality_freshness:"데이터 품질·최신성", bugs:"버그·오류", provider_fallback:"데이터 공급자·보조 경로",
  testing_regression:"테스트·회귀", ui_ux:"화면 구성", feature_effectiveness_simplification:"기능 실효성·단순화",
  continuity:"연속성", sensor_root_cause:"센서 문제 원인", news_source_independence:"뉴스 출처 독립성",
  contradiction_check:"반증 확인", issue_dedup:"중복 이슈 정리", code_change_attempt:"코드 변경 시도",
  news_full_scan:"뉴스 전체 점검", supervisor_continuity:"AI 연속성", canonical_data_quality:"정규 관측 데이터 품질",
  news_discovery:"뉴스 수집", issue_lifecycle:"이슈 흐름", long_term_memory:"장기 기억",
  safety_and_permissions:"안전·권한", discovery_source_degradation:"뉴스 수집축 저하",
  macro_runtime_empty:"거시 데이터 공백", canonical_window_recovery:"정규 관측 구간 복구"
};
const DISPLAY_COMPONENT_LABELS = {
  market:"시장 데이터", risk:"시장 위험", discovery:"뉴스 수집", supervisor:"AI 감독",
  provider:"데이터 공급자", regression:"회귀 검사", pages:"웹페이지", telegram:"텔레그램",
  health:"운영 상태", toss:"토스 데이터", naver_batch:"네이버 보조 데이터", macro:"거시 데이터"
};
function displayStatusLabel(value) {
  const raw = String(value == null || value === "" ? "UNKNOWN" : value).trim();
  const key = raw.replace(/[\s-]+/g, "_").toUpperCase();
  return DISPLAY_STATUS_LABELS[key] || raw;
}
function severityLabel(value) {
  return displayStatusLabel(value || "UNKNOWN");
}
function sessionStatusLabel(value) {
  return displayStatusLabel(value || "UNKNOWN");
}
function displayIdentifierLabel(value) {
  const raw = String(value || "").trim();
  if (!raw) return "";
  const key = raw.replace(/[\s-]+/g, "_").toLowerCase();
  return DISPLAY_IDENTIFIER_LABELS[key] || "";
}
function componentLabel(value) {
  const raw = String(value || "").trim();
  if (!raw) return "시스템";
  return DISPLAY_COMPONENT_LABELS[raw.toLowerCase()] || displayIdentifierLabel(raw) || "시스템";
}
function displayText(value) {
  const raw = String(value == null ? "" : value);
  if (!raw) return "";
  if (/^(?:https?:\/\/|[^\s]+\/(?:[^\s]+)|[^\s]+\.(?:json|jsonl|md|py|js|html|css|yml|yaml))$/i.test(raw)) return raw;
  const exact = displayStatusLabel(raw);
  if (exact !== raw) return exact;
  return raw
    .replace(/verification_pending/gi, "검증 대기")
    .replace(/no_change/gi, "변화 없음")
    .replace(/investigating/gi, "조사 중")
    .replace(/resolved_this_window/gi, "이번 구간에서 해결")
    .replace(/resolved/gi, "해결")
    .replace(/blocked/gi, "차단됨")
    .replace(/stale/gi, "갱신 지연")
    .replace(/healthy/gi, "정상")
    .replace(/recovered/gi, "복구됨")
    .replace(/degraded/gi, "일부 저하")
    .replace(/workflow_dispatch/gi, "수동 실행 요청")
    .replace(/canonical[- ]window/gi, "정규 관측 구간")
    .replace(/\bcanonical\b/gi, "정규 상태")
    .replace(/macro runtime/gi, "거시 데이터 실행 상태")
    .replace(/source_summary/gi, "수집 상태 요약")
    .replace(/\bdiscovery\b/gi, "뉴스 수집")
    .replace(/\bfallback\b/gi, "보조 경로")
    .replace(/\bprovider\b/gi, "데이터 공급자")
    .replace(/\bslot\b/gi, "관측 구간")
    .replace(/\bcompile\b/gi, "문법 검사")
    .replace(/\bpytest\b/gi, "테스트")
    .replace(/diff_check/gi, "변경사항 검사")
    .replace(/\bhistory\b/gi, "이력")
    .replace(/\bdetails\b/gi, "상세보기")
    .replace(/\bdesktop\b/gi, "데스크톱")
    .replace(/\bmobile\b/gi, "모바일");
}
function humanizeObject(value) {
  const item = asObject(value);
  if (!Object.keys(item).length) return "";
  if (item.item && item.resolution) {
    return displayText(item.item) + " — 처리: " + displayText(item.resolution);
  }
  if (item.a_position || item.b_position) {
    const parts = [];
    if (item.issue || item.title) parts.push(displayText(item.issue || item.title));
    if (item.a_position) parts.push("A 의견: " + displayText(item.a_position));
    if (item.b_position) parts.push("B 의견: " + displayText(item.b_position));
    if (item.evidence_needed) parts.push("추가 근거: " + displayText(item.evidence_needed));
    if (item.verify_after) parts.push("다음 확인: " + displayText(item.verify_after));
    return parts.join(" · ") || "의견 차이를 추가 확인 중입니다.";
  }
  const typeLabel = displayIdentifierLabel(item.type || item.axis || "");
  const status = item.status ? displayStatusLabel(item.status) : "";
  const head = item.title || item.name || item.term || item.focus || item.check || item.claim || item.text || "";
  const detail = item.detail || item.finding || item.signal || item.reason || item.why || item.summary || item.evidence || item.note || item.result || "";
  const parts = [];
  const intro = [typeLabel || displayText(head), status].filter(Boolean).join(" · ");
  if (intro) parts.push(intro);
  if (detail && displayText(detail) !== displayText(head)) parts.push(displayText(detail));
  if (item.action && item.action !== head) parts.push("조치: " + displayText(item.action));
  if (item.proposal) parts.push("개선안: " + displayText(item.proposal));
  if (item.resolution && !item.item) parts.push("처리: " + displayText(item.resolution));
  if (item.verify_after) parts.push("다음 확인: " + displayText(item.verify_after));
  if (parts.length) return parts.join(" — ");
  if (item.message) return displayText(item.message);
  return "세부 내용을 구조화해 표시할 수 없습니다.";
}
...
function sessionCard(row, market) {
  if (market === "kr") {
    return '<div class="session-card"><div class="date"><span>' + esc(row.date || "-") + '</span><span>' + esc(sessionStatusLabel(row.status)) + '</span></div>' +
...
function renderMetrics(data, krRows, usRows) {
  ...
  const cards = [
    ...
    ["데이터 상태", displayStatusLabel(health.status || "UNKNOWN"), (health.issue_count || 0) + "개 점검 항목"]
  ];
...
function renderHotIssues(data) {
  ...
  root.innerHTML=rows.length?rows.map((x,i)=>{
    ...
    return '<a class="hot-issue-card" href="#issue-'+esc(x.issue_id||i)+'" data-level="'+esc(status)+'">'+
      ...
      '<small>관련 기사 '+esc(articles||"-")+'건 · 매체 '+esc(publishers||"-")+'개 · 최근 증가 +'+esc(Math.max(0,velocity))+' · '+esc(displayStatusLabel(status))+'</small>'+
      '</div></a>';
  ...
  const render = () => {
    ...
    root.innerHTML = filtered.length ? filtered.map((x) => {
      ...
      return '<details id="issue-' + esc(x.issue_id||"unknown") + '" class="issue-detail" data-level="' + esc(status) + '" data-impact="' + esc(impact) + '">' +
        ...
        '<div class="issue-summary-meta"><span>' + esc(x.scope==="KOREA"?"국내":"글로벌") + '</span><span>' + esc(x.category||"") + '</span><b>' + esc(severityLabel(x.severity)) + '</b>' +
        ...
        '<div class="issue-expanded">' +
          ...
          '<div class="issue-detail-grid">' +
            ...
            '<section><h3>확인된 사실·근거</h3>' + (facts.length?'<ul>'+facts.map((v)=>'<li>'+esc(typeof v==="string"?v:(readableItem(v)))+'</li>').join("")+'</ul>':'<p class="muted">구조화된 사실 목록을 추가 확인 중입니다.</p>') + '</section>' +
            '<section><h3>미확인·반증 조건</h3>' + (unknowns.length?'<ul>'+unknowns.map((v)=>'<li>'+esc(typeof v==="string"?v:(readableItem(v)))+'</li>').join("")+'</ul>':'<p class="muted">현재 별도 미확인 항목 없음</p>') + '</section>' +
          '</div>' +
...
function renderResearch(data) {
  ...
  setText("supervisor-status", displayStatusLabel(supervisor.status || "UNKNOWN"));
  ...
  const supervisorTitle = supervisor.title || (supervisor.supervisor ? "AI " + supervisor.supervisor + " 최신 시장 리서치" : "최신 AI 심층 리서치");
  ...
  setHTML("supervisor-research", Object.keys(supervisor).length ? '<details class="research-detail">' +
    ...
    '<div class="research-meta"><span>' + esc(researchDateLabel({generated_at:supervisor.processed_at})) + '</span><span>' + esc(displayStatusLabel(supervisor.status)) + '</span>' +
    (supervisorFocus.length ? '<span>관련 이슈 ' + esc(supervisorFocus.length) + '개</span>' : '') + '</div><p>' + esc(shortText(supervisorPreview, 360)) + '</p>' +
    researchTags(supervisorFocus.slice(0,4).map((x)=>asObject(x).focus || asObject(x).title || asObject(x).name || "")) +
  ...
  setHTML("discovery-sources", sources.length ? sources.map(([name,v]) => {
    ...
    return '<div class="mini-row"><strong>' + esc(name) + '</strong><span>' + esc(displayStatusLabel(state.status || "UNKNOWN")) +
...
function renderAIDialogue(data) {
  ...
  setText("ai-dialogue-status", (stale ? "갱신 지연 · " : "최근 ") + (latest.processed_at ? relativeTime(latest.processed_at) : "시각 미확인"));
  ...
  root.innerHTML = rows.map((x) => {
    ...
    return '<article class="ai-turn ' + speakerClass + '">' +
      ...
      '<details class="ai-message">' +
        ...
        '<div class="ai-message-badges"><span>' + esc(displayStatusLabel(x.status)) + '</span>' + (completenessText ? '<span>' + esc(completenessText) + '</span>' : '') + '</div></div>' +
...
function renderRisk(data) {
  ...
  if (total && !known) rows.push({level:"UNKNOWN",title:"실시간 매크로 값 미수집",why:"빈 값을 낮은 위험으로 해석하지 않습니다. 실시간 값이 복구될 때까지 최근 종가와 AI 검증 메모를 함께 봅니다.",value:known+"/"+total});
  const score={VETO:5,HIGH:4,WATCH:3,UNKNOWN:2,LOW:1};
  rows.sort((a,b)=>(score[b.level]||0)-(score[a.level]||0));
  setHTML("overnight-risk-list", rows.length ? '<div class="risk-list">' + rows.slice(0,8).map((x,i) => '<div class="risk-card" data-level="' + esc(x.level) + '"><div class="risk-rank">' + (i+1) + '</div><div><strong>' + esc(x.title) + ' · ' + esc(displayStatusLabel(x.level)) + '</strong><p>' + esc(displayText(x.why)) + '</p><small>' + esc(x.value || "") + '</small></div></div>').join("") + '</div>' : empty("확인 가능한 오버나잇 리스크 입력이 없습니다."));
  const level = evaln.risk_level || risk.risk_level || "UNKNOWN";
  setText("risk-level", displayStatusLabel(level));
  if ($("risk-level")) $("risk-level").dataset.level = level;
  setHTML("risk-summary", '<div class="regime"><strong>' + esc(displayText(evaln.summary || "리스크 입력을 확인 중입니다.")) + '</strong><p>' + esc(((evaln.single_biggest_risk || {}).title) || "확정된 단일 대형 리스크 없음") + '</p><small>' + esc(displayText(((evaln.single_biggest_risk || {}).why) || "")) + '</small></div>');
...
function renderMacro(data) {
  const macro = data.macro_matrix || {};
  setText("macro-regime", displayStatusLabel(macro.macro_regime || macro.risk_level || "UNKNOWN"));
  ...
  setHTML("macro-groups", Object.entries(groups).map(([g,items])=>'<div><p class="subhead">' + esc(g) + '</p><div class="macro-cards">' + items.map((x)=>'<div class="macro-card"><strong>' + esc(x.label) + '</strong><b>' + esc(val(x)) + '</b><small>' + esc(displayStatusLabel(x.risk_level || "LOW")) + '</small></div>').join("") + '</div></div>').join(""));
}
function renderDomesticLive(data, krRows) {
  ...
  const labels={ok:"장중 실데이터",historical_fallback:"휴장 · 최근 3거래일 분석",outside_regular_session:"장 종료 · 최근 거래일 분석",needs_credentials:"API 설정 필요",no_rows:"실시간 시세 없음",toss_snapshot_unavailable:"토스 수집기 대기",outside_domestic_monitor_window:"국내 감시시간 종료",no_live_data:"최근 3거래일 모드"};
  setText("market-status", labels[status] || displayStatusLabel(status));
  if (q.status === "ok") {
    ...
    setHTML("burst-leaders",(q.burst_leaders||[]).length?(q.burst_leaders||[]).slice(0,8).map((x)=>'<div class="mini-row"><strong>' + esc(x.name||x.ticker) + '</strong><span>' + esc(x.burst_count||0) + '회 · 1분 최대 ' + esc(num(Number(x.max_minute_amount||0)/100000000,1)) + '억</span></div>').join(""):empty("현재 거래대금 급증 조건 종목 없음"));
...
function renderOperations(data) {
  ...
  setText("operations-status",displayStatusLabel(ops.status||"UNKNOWN"));
  ...
  setHTML("operations-summary",
    '<strong>자가복구 상태 · '+esc(displayStatusLabel(ops.status||"UNKNOWN"))+'</strong>'+
  ...
  setHTML("operations-kanban",columns.map((name)=>{
    ...
    return '<section class="ops-column"><h3>'+esc(name)+' <span>'+rows.length+'</span></h3><div class="ops-cards">'+
      (rows.length?rows.map((x)=>'<div class="ops-card"><strong>'+esc(x.title||"상태")+'</strong><p>'+esc(displayText(x.impact||""))+'</p><small>담당 '+esc(displayIdentifierLabel(x.owner)||displayText(x.owner||"자동화"))+(x.verify_after?' · 다음 '+esc(displayText(x.verify_after)):'')+'</small></div>').join(""):'<div class="ops-empty">없음</div>')+
      '</div></section>';
  }).join(""));
}
function renderSystem(data) {
  const provider=data.toss_provider||{}, runtime=data.market_runtime||{};
  const mode=provider.available?"토스 실시간":(runtime.provider==="kiwoom"?"키움 보조 경로":"연결 대기");
  const cards=[
    ["현재 경로",mode],["토스 최근 스냅샷",provider.available?relativeTime(provider.captured_at):"없음"],
  ...
  setText("health-status",displayStatusLabel(hs)); if($("health-status")) $("health-status").dataset.level=hs;
  setHTML("health-issues",(health.issues||[]).length?(health.issues||[]).map((x)=>'<div class="health-item" data-level="' + esc(x.severity||"") + '"><div><strong>' + esc(componentLabel(x.component)) + ' · ' + esc(severityLabel(x.severity)) + '</strong><p>' + esc(displayText(x.message||"")) + '</p>' + ((x.code||x.component)?'<details class="inline-disclosure"><summary>개발자용 원본 정보</summary><div class="source-line">구성요소 ' + esc(x.component||"미확인") + ' · 코드 ' + esc(x.code||"미확인") + '</div></details>':'') + '</div><div class="health-recovery">' + esc(displayText(x.recovery||"")) + '</div></div>').join("") : '<div class="notice">현재 확인된 운영 이상 없음</div>');
  const reg=data.regression||{}, snap=reg.snapshot||{}, base=reg.baseline_summary||{};
  setText("regression-status",displayStatusLabel(reg.status||"COLLECTING"));
  ...
  setHTML("regression-evals",(reg.evaluations||[]).length?(reg.evaluations||[]).slice(0,8).map((x)=>'<div class="mini-row"><strong>변경 검증</strong><span>' + esc(displayStatusLabel(x.status)) + (x.drop_7d!=null?" · 7일 "+Number(x.drop_7d).toFixed(1)+"p":"") + '</span>' + (x.change_id?'<details class="inline-disclosure"><summary>개발자용 원본 정보</summary><div class="source-line">변경 ID ' + esc(x.change_id) + '</div></details>':'') + '</div>').join("") : empty("회귀 평가 표본을 수집 중입니다."));
  setHTML("recent-changes",(data.recent_changes||[]).length?(data.recent_changes||[]).slice(0,15).map((x)=>'<div class="tick"><span class="dot"></span><div><strong>' + esc(x.title||"AI 변경") + '</strong><p>' + esc((x.paths||[]).join(", ")) + '</p><div class="meta">' + esc(x.applied_at||"") + ' · ' + esc(displayStatusLabel(x.status)) + '</div>' + (x.change_id?'<details class="inline-disclosure"><summary>개발자용 원본 정보</summary><div class="source-line">변경 ID ' + esc(x.change_id) + '</div></details>':'') + '</div></div>').join("") : empty("최근 변경 기록 없음"));
  setHTML("ticks",(data.ticks||[]).length?(data.ticks||[]).slice(0,15).map((x)=>'<div class="tick"><span class="dot"></span><div><strong>' + esc(x.title||"판단") + '</strong><p>' + esc(x.observation||"") + '</p><div class="meta">' + esc(x.time||"") + ' · ' + esc(displayStatusLabel(x.outcome)) + '</div></div></div>').join("") : empty("AI 진화 Tick 기록 없음"));
