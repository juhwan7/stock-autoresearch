...
3. Bot Token과 해당 개인 채팅의 Chat ID를 확인한다.
4. GitHub 저장소 → `Settings → Secrets and variables → Actions`에서 다음 Repository secret 두 개를 만든다.
...
`Actions → 텔레그램 알림 테스트 → Run workflow`
