...
현재 기본 구조는 GitHub Actions가 10분마다 AI 없이 관측을 저장하고, ChatGPT 예약 작업이 관측 6개를 1시간 단위로 처리한다. 따라서 GitHub Repository Secret에 `OPENAI_API_KEY`를 넣지 않아도 된다.
...
`.github/workflows/연속연구와진화.yml`이 매 10분 snapshot을 받아 감독 관측에 반영한다.
...
목적: 1시간 감독 배치에서 실제 개선이 적용되거나 중요한 확인 사항이 생겼을 때 다음 10분 GitHub Actions를 통해 휴대폰 Telegram으로 알림을 받는다.
