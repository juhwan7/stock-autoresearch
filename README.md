...
GitHub Actions의 6분 센서는 OpenAI API를 호출하지 않습니다. AI 판단과 웹 심층조사는 별도의 매시간 ChatGPT 감독이 담당합니다.
...
`.github/workflows/연속연구와진화.yml`이 6분마다 시장과 프로젝트 상태를 관측합니다.

주요 입력은 네이버 공개 6분 배치, 연결된 Toss 데이터, 시장 discovery, DART·뉴스 단서, Risk, Health, Regression입니다. 센서는 AI 없이 객관적인 관측과 질문 후보를 저장합니다.
...
자세한 정의는 [시장데이터 명세](docs/시장데이터_명세.md), Toss 설정은 [고정 IP Toss 6분 러너](docs/고정IP_토스_러너.md)를 봅니다.
...
장중에는 Top50 개수, 1분봉 공백, 종목별 fetch 오류, 보조 공급자와의 종목 차이, 6분 실행 지연, Top50 이탈 종목 추적 단절 등을 검사합니다.
...
| `.github/workflows/` | 6분 센서·테스트·배포·알림 |
